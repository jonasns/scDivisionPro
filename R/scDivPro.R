#' Single cell division profiling function
#'
...
#' @param division_peaks_graphs_list List of division peaks graphs
...
#' @param moi2 2nd marker of interest to use for scatter plot (default is Ki67).
#' @param soi_list A vector containing all subsets of interest.
#' @return A list containing SCE data and graphs related to CFSE analysis.
#' @import ggplot2
#' @importFrom reshape2 melt
#' @importFrom ggridges geom_density_ridges theme_ridges
#' @importFrom viridis viridis
#' @importFrom ggplot2 geom_area geom_vline ggtitle
#' @importFrom dplyr filter
#' @export
scDivPro <- function(sce, donor_list, division_peaks_graphs_list, bd, dn, num_peaks = 5, peak_boundary_uncertainty = 0.2, approximate_peak_distance = 1, condition = "stim_0x_Treg", moi = "CFSE", moi2 = "Ki67", soi_list = c("CD8_CM", "CD8_EM", "Th1", "eTreg")) {
  ...
  div_levels <- findInterval(
    sce_temp1@assays@data$exprs[moi, ],
    sorted_differences,
  ...
  result_list <- list(sce = sce_temp1, graphs = list())
  ...
  for (j in seq_along(soi_list)) {
    soi <- soi_list[j]

    if (table(sce_temp1$condition %in% condition & sce_temp1$subsets == soi)[2] > 1 | length(table(sce_temp1$condition %in% condition & sce_temp1$subsets == soi)) == 1) {
      sce_use <- filterSCE(sce_temp1, condition %in% condition & subsets == soi)
    } else {
      sce_use <- sce_temp1
    }
    ...
    pp <- ggplot(new_tab2, aes(x = x, y = density)) + 
      ...
      labs(x = paste0(moi," signal (asinh)"), y = "density") +
      ggtitle(paste0(donor, "_", soi))
    
    ppp <- ggplot(new_tab, aes(y = value, x = div)) +
      ...
      scale_fill_manual(values = viridis(num_peaks)) +
      theme_bw(base_size = 14) +
      theme(legend.position = "none", plot.title = element_text(size = 14), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
      ggtitle(paste0(donor, "_", soi)) +
      labs(y = paste0(moi, " signal (asinh)"), x = "")

    pppp <- plotScatter(sce_use, c(moi, moi2), assay = "exprs", label = "both", bins = 80) + 
      ggtitle(paste0(donor, "_", soi)) + 
      geom_vline(xintercept = line_intervals, color = "black", linetype = "dashed")

    result_list$graphs[[soi]] <- list(density_plot = pp, violin_plot = ppp, scatter_plot = pppp)
